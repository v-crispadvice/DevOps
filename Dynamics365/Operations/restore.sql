...
DECLARE @MSA varchar(255)
SELECT @MSA =  DEFAULT_DOMAIN() + '\' + @@SERVERNAME + '$'

IF NOT EXISTS (SELECT name FROM master.sys.server_principals WHERE [NAME] = @MSA)
BEGIN
 EXECUTE ('CREATE LOGIN "'+@MSA+'" FROM WINDOWS WITH DEFAULT_DATABASE=[master], DEFAULT_LANGUAGE=[us_english];');
 EXEC ('ALTER SERVER ROLE [sysadmin] ADD MEMBER "' + @MSA + '"')
End
GO
